...
|---|---|---|---|
| T-1150 | v1 core loop — greenfield build | in progress | This build. Server + client per v1 spec. |
| T-1151 | Create GitHub remote + push | todo | Eli creates the repo; no remote configured yet. Then `git remote add origin … && git push -u origin main`. |
...
| T-1153 | Provision Supabase project + run migrations | todo | Create project, run `server/db/migrations/*.sql`, create private `videos` bucket, set env vars. Eli (manual). |
